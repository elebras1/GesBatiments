...
/**
 * La classe {@code BureauxVisiteur} implémente l'interface {@link Visiteur} et fournit une
 * représentation textuelle des éléments visités, tels que les bâtiments, les étages, et les pièces.
 *
 * <p>
 * Cette classe utilise le design pattern Visiteur pour parcourir la structure des objets
 * {@link Batiment}, {@link Etage} et {@link Piece} et générer une description formatée.
 */
public class BureauxVisiteur implements Visiteur {
  private StringBuilder result = new StringBuilder();

  /**
   * Le {@code StringBuilder} utilisé pour construire la représentation textuelle des éléments
   * visités.
   */
  @Override
  public void visite(Batiment batiment) {
    this.result.append("Bâtiment ").append(batiment.getNom()).append(" (")
        .append(batiment.getNumero()).append(")\n");
  }
  ...
  public void visite(Piece piece) {
    this.result.append("        Pièce ").append(piece.getNumero()).append(" - Type: ")
        .append(piece.getEstBureau() ? "Bureau" : "Autre").append("\n");
  }
