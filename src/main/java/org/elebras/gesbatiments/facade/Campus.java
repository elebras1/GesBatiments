...
/**
 * Représente un campus composé de plusieurs bâtiments. Le campus permet d'ajouter, de modifier, de
 * supprimer et d'afficher des bâtiments. Il prend également en charge l'observation des changements
 * et la vérification des détails des bâtiments.
 */
public class Campus implements Observable {
  ...
  /**
   * Constructeur de la classe {@code Campus}. Initialise l'ensemble des bâtiments, les fabriques de
   * bâtiments et la liste des observateurs.
   */
  public Campus() {
  ...
  /**
   * Ajoute un bâtiment au campus. Vérifie si le bâtiment existe déjà et si les paramètres sont
   * valides. Si le bâtiment est valide, il est ajouté à l'ensemble des bâtiments du campus.
   *
   ...
   */
  public AjouterBatimentResultat ajouterBatiment(String nom, int nbPieceParEtage, int nbBureau,
      int surfacePiece) {
  ...
  /**
   * Ajoute un ensemble de bâtiments à partir d'un fichier JSON. Vérifie si les bâtiments existent
   * déjà et si les paramètres sont valides. Si les bâtiments sont valides, ils sont ajoutés à
   * l'ensemble des bâtiments du campus.
   *
  ...
  public Map<String, VerificationResultat> ajouterBatiments(File file) {
    ...
    for (Batiment batiment : batimentsConstruits) {
      boolean nomExistant =
          this.batiments.stream().anyMatch(b -> b.getNom().equals(batiment.getNom()));
  ...
  /**
   * Modifie le nom d'un bâtiment existant. Vérifie si le nom du bâtiment est déjà pris. Si le nom
   * est valide, il est modifié pour le bâtiment correspondant.
   *
  ...
  public String modifierNomBatiment(int numeroBatiment, String nomBatiment) {
    for (Batiment batimentVoisin : this.batiments) {
      if (!batimentVoisin.getNumero().equals(numeroBatiment)
          && nomBatiment.equals(batimentVoisin.getNom())) {
        return null;
      }
    }
  ...
  /**
   * Supprime un bâtiment du campus. Vérifie si le bâtiment existe et le supprime de l'ensemble des
   * bâtiments du campus.
   *
  ...
  public String toString() {
    return "Campus{" + "batiments=" + batiments + ", factory=" + factory + '}';
  }
}
